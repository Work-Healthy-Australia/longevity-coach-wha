...
| BUG-018 | P1 | 1 | `signIn()` discards `?redirect=` query param after successful login | /login | 2026-05-01 | OPEN | [app/(auth)/actions.ts:35](app/(auth)/actions.ts:35) hard-codes `redirect("/dashboard")`. proxy.ts sets `?redirect=/uploads` etc. on auth-required redirects but the form ignores it. Users land on dashboard not the page they tried to open. |
...
| BUG-024 | P2 | 1 | Inconsistent post-login redirect preservation across guarded routes | /insights, /journal, /care-team, /alerts, /routines | 2026-05-01 | OPEN | These routes are NOT in `PROTECTED_PREFIXES` in [lib/supabase/proxy.ts:4](lib/supabase/proxy.ts:4). They use page-level `redirect("/login")` (without a `redirect=` query param). Result: user opens `/insights` while logged out → bounces to bare `/login` → after sign-in lands on `/dashboard` not `/insights`. Other guarded routes correctly preserve the destination. Fix: add these prefixes to `PROTECTED_PREFIXES` and remove the page-level redirects (per CLAUDE.md). |
