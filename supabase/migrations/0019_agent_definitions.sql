-- Agent definitions table: stores system prompts, model configs, and MCP server connections
-- per agent. Runtime agents load their definition at request time — no redeploy needed
-- for prompt/model changes made via the admin Agent Manager UI.

CREATE TABLE IF NOT EXISTS agent_definitions (
  id          uuid        PRIMARY KEY DEFAULT gen_random_uuid(),
  slug        text        NOT NULL UNIQUE,          -- 'janet' | 'alex' | 'atlas' | 'sage'
  ...
  -- mcp_servers shape: [{ id: uuid, name: string, type: 'sse'|'stdio', url: string, enabled: boolean }]
...
-- Only service_role (admin SDK) can write; admin UI uses admin client
CREATE POLICY "service_role_all_agent_definitions"
  ON agent_definitions FOR ALL
  USING (true)
  WITH CHECK (true);

-- Seed initial agent definitions
INSERT INTO agent_definitions (slug, display_name, description, model, system_prompt, max_tokens)
VALUES
  (
    'janet',
    'Janet',
